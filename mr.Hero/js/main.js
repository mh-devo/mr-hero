...
$(document).ready(function () {
    ...
    // navbar mob icon on click change background color
    ...
    $(".header .name").css('padding-top', ($(window).height() / 2) - ($(".header .name").height() / 2) + "px");
});
//              Start Canvas Skills
...
function peopleOpinion() {
    ...
    var containerWidth = $(".people-container").width(),
        ...
        peopleCount = $(".people-opinion").length,       // length = 9
        clicksNum = Math.round(peopleCount / 4.5), // 4.5 = 1 click (show 3 people opinions)
        pre = $(".fa-angle-left"),
        next = $(".fa-angle-right"),
        clicks = 1;
    if ($(window).innerWidth() <= 991 && $(window).innerWidth() > 699) {
        clicksNum = Math.round(peopleCount / 2.5);  // 2.5 = 1 click (show 2 people opinions)
    } else if ($(window).innerWidth() <= 699) {
        clicksNum = Math.round(peopleCount / 1.2);  // 1.2 = 1 click (show 1 people opinion)
    } else {
        clicksNum = Math.round(peopleCount / 4.5);
    }
